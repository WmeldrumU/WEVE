...
namespace weve {
        ...
        WevePipeline::~WevePipeline() {
            vkDestroyShaderModule(weveDevice.device(), vertShaderModule, nullptr);
            vkDestroyShaderModule(weveDevice.device(), fragShaderModule, nullptr);
            vkDestroyPipeline(weveDevice.device(), graphicsPipeline, nullptr);
        }
        ...
        void WevePipeline::createGraphicsPipeline(const std::string& vertFilepath, const std::string& fragFilepath, const PipelineConfigInfo& configInfo) {
            ...
            auto attributeDescriptions = WeveModel::Vertex::getAttributeDescriptions();
            auto bindingDescriptions = WeveModel::Vertex::getBindingDescriptions();
            ...
            pipelineInfo.pDynamicState = nullptr; //used to dynamically change pipeline attributes
            ...
            //used for derriving one pipeline from another
            pipelineInfo.basePipelineIndex = -1;
            pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;

            if (vkCreateGraphicsPipelines(weveDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
        ...
        void WevePipeline::bind(VkCommandBuffer commandBuffer) {
        ...
        PipelineConfigInfo WevePipeline::defaultPipelineConfigInfo(uint32_t width, uint32_t height) {
            ...
            //scissor removes anything that is in the viewport,
            //but is outside of its extent
